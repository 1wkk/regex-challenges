...
export const REPO = 'https://github.com/echoeureka/regex-challenges'
...
export function toRawREADME(quiz: Ch, locale?: string) {
  const provider = 'https://cdn.jsdelivr.net/gh/echoeureka/regex-challenges'
  // const provider = 'https://raw.githubusercontent.com/echoeureka/regex-challenges/master'
  return locale && locale !== defaultLocale && quiz.readme[locale]
...
export function toShareAnswerFull(quiz: Ch, locale: string = defaultLocale) {
  ...
  if (locale === defaultLocale)
    return `https://github.com/echoeureka/regex-challenges/issues/new?labels=answer,${encodeURIComponent(
      locale
